...
class CandidatesSelection:
    ...
    def select_the_youngest_and_the_most_skills_candidate_with_him_key(self):
        ...
        for cookie in cookies:
            ...
            self.driver.refresh()
            wait(self.driver, 5).until(EC.visibility_of_element_located((By.ID, 'age')))

            age_input = self.driver.find_element(By.ID, 'age').text
            age_input_parts = age_input.split(' ')
            age_string = age_input_parts[-1]
            age = int(age_string)

            skill_inputs = self.driver.find_elements(By.TAG_NAME, 'li')
